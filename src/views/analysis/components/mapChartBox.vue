...
export default {
  ...
  watch: {
    optionData: function() {
      ...
      if (this.optionData.isMapCities) {
        var citiesData = this.optionData.option.series[0].data
        this.optionData.option.series[0].data = this.convertCitiesData(
          citiesData
        )
      }
  ...
  methods: {
    convertCitiesData(citiesData) {
      var res = []
      for (var i = 0; i < citiesData.length; i++) {
        var geoCoord = this.citiesJson[citiesData[i].name]
        if (geoCoord) {
          res.push({
            name: citiesData[i].name,
            value: geoCoord.concat(citiesData[i].value)
          })
        }
      }
      return res
